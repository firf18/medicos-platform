...
export default function FarmaciasLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: pharmacy, error: pharmacyError } = await (supabase as any)
        .from('pharmacies')
        .select('id')
        .eq('id', data.user.id)
        .single()
      ...
      if (pharmacyError || !pharmacy) {
        ...
        throw new Error('Esta cuenta no es de farmacia. Usa el login correspondiente a tu tipo de cuenta.')
      }

      // Verificar si completó la configuración
      const { data: pharmacyConfig } = await (supabase as any)
        .from('pharmacies')
        .select('name, address, phone')
        .eq('id', data.user.id)
        .single()

      const config = pharmacyConfig as { name?: string; address?: string; phone?: string } | null
      const needsSetup = !config || !config.name || !config.address
      
      if (needsSetup) {
        router.push('/auth/setup-wizard/pharmacy')
      } else {
      ...
      <div className="max-w-md w-full space-y-8">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-gray-600">
            Accede al sistema de gestión de tu farmacia
          </p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-8">
          <form className="space-y-6" onSubmit={handleLogin}>
            ...
            <div className="text-center space-y-2">
              ...
              >
                ¿No tienes cuenta? Regístrate como farmacia
              </Link>
